...
END:VCARD
        ...
        fakeReply: {
            ...
            value(jid, text = '', fakeJid = this.user.jid, fakeText = '', fakeGroupJid, options) {
                return conn.reply(jid, text, { key: { fromMe: fakeJid == conn.user.jid, participant: fakeJid, ...(fakeGroupJid ? { remoteJid: fakeGroupJid } : {}) }, message: { conversation: fakeText }, ...options })
            }
        },
        downloadM: {
            /**
                    ...
                    try {
                        ...
                        if (message.quoted) {
                            ...
                            if ((remoteJid && remoteJid == ! 'status@broadcast') && quoted) {
                                ...
                                quoted[qMtype].contextInfo.mentionedJid = message.quoted.mentionedJid || quoted[qMtype].contextInfo.mentionedJid || []
...
export function smsg(conn, m, hasParent) {
    ...
    m = M.fromObject(m)
    if (m.key) m.sender = conn.decodeJid(m.fromMe && conn.user.id || m.participant || m.key.participant || m.chat || '')
    if (m.message) {
        ...
        const quoted = m._quoted
        if (quoted) {
            m.quoted = m.getQuoted(conn)
    ...
    if (m.mediaMessage) {
        ...
        m.download = (saveToFile = false) => conn.downloadM(m.mediaMessage[mtype], mtype.replace(/message/i, ''), saveToFile)
    }
    /**
    ...
    try {
        if (m.msg?.key && m.mtype == 'protocolMessage') conn.ev.emit('message.delete', m.msg.key)
    } catch (e) {
        console.error(e)
    }
    return m
...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        fromMe: {
            get() {
                return this._fromMe || this.key?.fromMe || false
            },
            set(fromMe) {
                return this._fromMe = fromMe || this.fromMe
            }
        },
        mtype: {
            get() {
        ...
        _quoted: {
            get() {
                const quoted = this.msg?.contextInfo?.quotedMessage
                return quoted || null
            }
        },
        getQuoted: {
            ...
            value(conn) {
                ...
                let q = quoted[type]
                return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? { _text: q } : q)), {
                    mtype: {
                        get() {
                            return type
                        },
                    ...
                    text: {
                        get() {
                            return this._text || this.caption || this.contentText || ''
                        },
                        enumerable: true
                    },
                    mentionedJid: {
                    ...
                    getQuotedObj: {
                        async value() {
                            ...
                            const q = M.fromObject(await conn.loadMessage(this.id) || this.vM)
                            return smsg(conn, q)
                        },
                        enumerable: true,
                    ...
                    forward: {
                        ...
                        value(jid, forceForward = false) {
                            return conn.forwardMessage(jid, this.vM, forceForward)
                        },
                        enumerable: true,
                    },
                    copyNForward: {
                        ...
                        value(jid, forceForward = true, options = {}) {
                            return conn.copyNForward(jid, this.vM, forceForward, options)
                        },
                        enumerable: true,
                    ...
                    cMod: {
                        ...
                        value(jid, text = '', sender = this.sender, options = {}) {
                            return conn.cMod(jid, this.vM, text, sender, options)
                        },
                        enumerable: true,
                    ...
                    delete: {
                         ...
                         */
                        value() {
                            return conn.sendMessage(this.chat, { delete: this.vM.key })
                        },
                        enumerable: true,
